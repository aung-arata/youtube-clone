          ...
          {libraryItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-6 px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-sm dark:text-gray-200 ${
                ...
                isActive(item.path) && item.label === 'Playlists' ? 'bg-gray-100 dark:bg-gray-700 font-semibold' : ''
              } ${
                isActive(item.path) && item.label === 'Notifications' ? 'bg-gray-100 dark:bg-gray-700 font-semibold' : ''
              }`}
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {item.icon === 'library' && (
