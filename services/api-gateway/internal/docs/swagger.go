...
// GetOpenAPISpec returns the OpenAPI specification for the YouTube Clone API Gateway
func GetOpenAPISpec() *OpenAPISpec {
	return &OpenAPISpec{
		OpenAPI: "3.0.3",
		Info: SwaggerInfo{
			Title:       "YouTube Clone API Gateway",
			Description: "API Gateway for the YouTube Clone microservices architecture. Routes requests to appropriate backend services.",
			Version:     "1.0.0",
			BasePath:    "/api",
		},
		Servers: []Server{
			{URL: "http://localhost:8080", Description: "API Gateway (Development)"},
			{URL: "https://api.yourdomain.com", Description: "Production API Gateway"},
		},
		Tags: []Tag{
			{Name: "Authentication", Description: "User authentication (routed to User Service)"},
			{Name: "Videos", Description: "Video management (routed to Video Service)"},
			{Name: "Comments", Description: "Comments (routed to Comment Service)"},
			{Name: "Users", Description: "User profiles (routed to User Service)"},
			{Name: "History", Description: "Watch history (routed to History Service)"},
			{Name: "Notifications", Description: "Notifications (routed to Notification Service)"},
			{Name: "Playlists", Description: "Playlists (routed to Video Service)"},
			{Name: "Subscriptions", Description: "Subscriptions (routed to User Service)"},
			{Name: "Plans", Description: "Subscription plans (routed to User Service)"},
			{Name: "Gateway", Description: "API Gateway specific endpoints"},
		},
		Paths:      buildGatewayPaths(),
...
func buildGatewayPaths() map[string]PathItem {
	return map[string]PathItem{
		"/api/auth/signup": {
			"post": Operation{
				Tags:        []string{"Authentication"},
				Summary:     "Register a new user",
				Description: "Proxied to User Service",
		...
		"/api/videos": {
			"get": Operation{
				...
				Description: "Proxied to Video Service",
				...
				Responses: map[string]Response{
					...
					"502": {Description: "Video Service unavailable"},
				},
			},
		},
		"/api/videos/{id}": {
		...
		"/api/users/{userId}/notifications": {
			"get": Operation{
				...
				Description: "Proxied to Notification Service",
				Parameters: []Parameter{
					{Name: "userId", In: "path", Required: true, Schema: &Schema{Type: "integer"}},
		...
		"/api/users/{userId}/notifications/ws": {
			"get": Operation{
				Tags:        []string{"Notifications"},
				Summary:     "WebSocket connection for real-time notifications",
				Description: "Establishes WebSocket connection for real-time notification updates. Proxied to Notification Service.",
				Parameters: []Parameter{
					{Name: "userId", In: "path", Required: true, Schema: &Schema{Type: "integer"}},
				},
				Responses: map[string]Response{
		...
		"/api/health": {
			"get": Operation{
				...
				Responses: map[string]Response{
					"200": {Description: "Gateway is healthy", Content: map[string]MediaType{"application/json": {Schema: &Schema{Ref: "#/components/schemas/ServiceStatus"}}}},
				},
			},
		},
	}
...
<head>
    ...
    <title>YouTube Clone API Gateway Documentation</title>
